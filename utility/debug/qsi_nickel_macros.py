...
def chip_phase_sweep(stepsize=50, verbose=0):
    """
    Sweep the 'chip' clock phase offset then plot the results.
    
    This method will sweep the phase offset of all 'chip' PLL outputs.  At
    each phase offset a frame is captured and the statistics calculated.  The
    statistics are plotted when the sweep is completed.
    
    The statistics include median, mean, standard deviation, number of codes
    with 0 counts, less than 10 counts, less than half the median counts 
    and greater than twice the median counts.
    
    This was initially created to visualize the affects of ADC clock phase 
    offset to the clustering of ADC values.  It is used in conjuction with the
    with the Q9001 ramp function.
    
    Arguments:
        stepsize   - Number of phase steps to increment for each iteration
        verbose    - Display progress dots,  0=Nothing displayed, 1=Display
    ...
    hist_zero = []
    hist_under_10 = []
    hist_under_half_median = []
    hist_over_two_median = []
    
    # Set the phase offset for all CHIP SI5338 outputs to 0
    phase_offset = [0,0,0,0]
    qsi.set_clk_offsets('CHIP', 0xF, phase_offset)

    # Step the phase offset though approximately one laser pulse period
    x = range(0, 5000, stepsize)
    
    for clk_offset in x:
        # Display progress indication if requested
        if (verbose != 0) and ((clk_offset % 10) == 0):
            print('.', end = '')

        phase_offset = [clk_offset, clk_offset, clk_offset, clk_offset]
        qsi.set_clk_offsets('CHIP', 0xF, phase_offset)

        # Capture one frame.  Data is indexed [frame,sample_phase,row,column]
        capture_frame = qsi.capture_raw(1,'raw')
        time.sleep(0.2)
        frame_mean.append( np.mean(capture_frame) )

        # Select one sample phase from the frame
        capture_sample = capture_frame[0,0,:,:]
        sample_mean.append( np.mean(capture_sample) )
        
        # Aggregate the values into histogram bins over the possible ADC range
        # but ignore 0 and full scale (1023).  
        (n, bins) = np.histogram(capture_sample,bins=np.arange(1,1022,1), density=False)

        # Calculate the statistics of the histogram    

        median = np.median(n)
        half_median = 0.5 * median
        double_median = 2 * median
        
        hist_median.append( median )
        hist_mean.append( np.mean(n) )
        hist_std.append( np.std(n) )
        hist_zero.append( np.sum(n == 0) )
        hist_under_10.append( np.sum(n < 10) )
        hist_under_half_median.append( np.sum(n < half_median) )
        hist_over_two_median.append( np.sum(n > double_median) )
        
    # Restore the phase offset for CHIP SI5338
    qsi.set_clk_offsets('CHIP', 0xF, [0,0,0,0])

    # Create the plots of the results
    plt.figure(figsize=[12,6])

    # The values for the  '< 0.5x median counts' are much larger than the others,
    # so plot it in it's own graph.
    
    # Create a 2 rows x 1 column figure and start working on the first axis
    # Include the plots that have higher values
    ax = plt.subplot(211)
    #ax.plot(x, np.array(hist_median), label='median counts')
    #ax.plot(x, np.array(hist_mean), label='mean counts')
    #ax.plot(x, np.array(hist_std), label='stddev counts')
    ax.plot(x, np.array(hist_zero), label='zero counts')
    ax.plot(x, np.array(hist_under_10), label='< 10 counts')
    ax.plot(x, np.array(hist_under_half_median), label='< 0.5x median counts')
    ax.plot(x, np.array(hist_under_half_median), label='< 0.5x median counts')
    ax.plot(x, np.array(hist_over_two_median), label='> 2x median counts')
    plt.title("ADC Clock Offset Sweep With Q9001 Ramp")

    # The tick labels for both axis are the same, so prevent x-axis tick labels
    # (by setting tick labels to null)
    ax.set_xticklabels([])
    # Get the location and size of the axes
    chartBox = ax.get_position()
    # Adjust the height and anchor position so that this plot fills the top half of default position
    ax.set_position([chartBox.x0, chartBox.y0 + (0.5 * chartBox.height), chartBox.width, (chartBox.height*0.5)])
    ax.legend()
    ax.grid()

    # Make plot for smaller values
    # Get the second axes
    ax = plt.subplot(212)
    ax.plot(x, np.array(hist_median), label='median counts')
    ax.plot(x, np.array(hist_mean), label='mean counts')
    ...
    plt.xlabel('Phase Steps')
    plt.ylabel('Counts')

    # Adjust vertical anchor to make space for legend below plot and 
    # adjust height so that this axis uses remainder of plot height (135% original)
    chartBox = ax.get_position()
    ax.set_position([chartBox.x0, chartBox.y0 + 0.1, chartBox.width, (chartBox.height*1.35)])
...
def set_timing_clock_freq_plan(component:str = "MCLK", ref_source:int = qsi.lib.LIBQSI_REFSRC_OSCILLATOR,
    ...
    ref_source : int, optional
        The input reference source.  The default is qsi.lib.LIBQSI_REFSRC_OSCILLATOR.
    ...
    Returns
    -------
    rc: int (qsi.lib.libqsi_status_t)
        status 

    Usage
    -----
        set_timing_clock_freq_plan("MCLK", qsi.lib.LIBQSI_REFSRC_OSCILLATOR, 10000000, 67080000, 67080000, 67080000, 67080000) 
    """
    component_id = qsi.text2cffi(component)

    freq_plan = qsi.ffi.new('libqsi_freq_plan_v2_config_t *')
    ...
    freq_plan.mode[0] = qsi.lib.LIBQSI_OUTPUT_MODE_PLL
    freq_plan.mode[1] = qsi.lib.LIBQSI_OUTPUT_MODE_PLL
    freq_plan.mode[2] = qsi.lib.LIBQSI_OUTPUT_MODE_PLL
    freq_plan.mode[3] = qsi.lib.LIBQSI_OUTPUT_MODE_PLL
    ...
    rc = qsi.lib.quantum_timing_clock_set_frequency_plan(qsi.lib.LIBQSI_DEVICE_NIOS, component_id, freq_plan)
    return rc 
...
def gsl_clock_init():
    ...
    """
    set_timing_clock_freq_plan("MCLK", qsi.lib.LIBQSI_REFSRC_OSCILLATOR, 10000000, 65000000, 8125000, 65000000, 65000000)
    set_timing_clock_freq_plan("GSL", qsi.lib.LIBQSI_REFSRC_OSCILLATOR,  65000000, 8125000, 8125000, 8125000, 8125000)
    qsi.set_clk_offsets("GSL", 0xF, [0, 150, 0, 0])
...
def gsl_enable(enable = 0, vlaser = 18.0, vbias = 0.4):
    ...
    """
    qsi.lib.quantum_gsl_laser_voltage_set(vlaser)
    qsi.lib.quantum_gsl_gate_bias_set(vbias)
    qsi.lib.quantum_set_laser_enabled(enable)
...
def gsl_info():
    ...
    laser_diode_setpoint  = qsi.ffi.new('float *')
    laser_diode_voltage  = qsi.ffi.new('float *')
    laser_diode_current  = qsi.ffi.new('float *')
    photodiode_current  = qsi.ffi.new('float *')
    gate_bias_voltage = qsi.ffi.new('float *')
    
    qsi.lib.quantum_gsl_laser_voltage_get(laser_diode_setpoint, laser_diode_voltage, laser_diode_current);
    qsi.lib.quantum_gsl_photodiode_current_get(photodiode_current)
    qsi.lib.quantum_gsl_gate_bias_get(gate_bias_voltage)
    ...
    gsl_csr  = qsi.ffi.new('gsl_csr_t *')

    qsi.lib.quantum_gsl_csr_get(gsl_csr)
    ...
    # Get the states flag
    state_flags = qsi.ffi.new('uint32_t *')
    rc = qsi.lib.quantum_get_device_state_flags(state_flags)
    if rc == qsi.lib.LIBQSI_STATUS_SUCCESS:
        print('  NIOS Lid      = {0:s}'.format('OPEN' if (state_flags[0] & qsi.lib.STATE_FLAG_LID_OPEN) else 'close'))
    else:
